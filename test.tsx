...
function validateCart(expected: { [key: number]: number }): void {
  for (const itemId in expected) {
    ...
    const productName = mockProducts.find((p) => p.id === parseInt(itemId))
      ?.name.uk;

    if (productName) {
      const itemCountElement = screen.getByText(
        new RegExp(`${productName} count:`, "i")
      );
      expect(itemCountElement).toHaveTextContent(
        new RegExp(`^${productName} count: ${expectedCount}$`)
      );
    }
  }
}
...
describe("Home", () => {
  const renderHome = async () => {
    ...
    return render(<CartProvider>{HomeComponent}</CartProvider>);
  };
  ...
  it("renders a basket with 1 item", async () => {
    ...
    const product1Button = screen.getByRole("button", {
      name: `Add ${mockProducts[0].name.uk} to basket`,
    });
  ...
  it("renders a basket with multiple items from different items", async () => {
    ...
    const product2Button = screen.getByRole("button", {
      name: `Add ${mockProducts[1].name.uk} to basket`,
    });
  ...
  it("presents localised prices when in us", async () => {
    ...
    mockProducts.forEach(function (product) {
      ...
      expect(
        screen.queryByText(`£${product.price.gbp}`)
...
describe("Checkout", () => {
  ...
  it("cart shows quanity ", async () => {
    ...
    const product1NameInCart = screen.getByRole("heading", {
      name: mockProducts[0].name.uk,
      level: 3,
    });
    const product1CheckoutLine = product1NameInCart.closest(
      ".checkoutLine"
    ) as HTMLElement;

    const product1QuantityContainer =
      within(product1CheckoutLine).getByText(/Quantity:/i).parentElement!;
    expect(product1QuantityContainer).toHaveTextContent("2");

    const product1SubtotalContainer =
      within(product1CheckoutLine).getByText(/Subtotal:/i).parentElement!;
    expect(product1SubtotalContainer).toHaveTextContent(
      String(mockProducts[0].price.gbp * 2)
    );

    const product2NameInCart = screen.getByRole("heading", {
      name: mockProducts[1].name.uk,
      level: 3,
    });
    const product2CheckoutLine = product2NameInCart.closest(
      ".checkoutLine"
    ) as HTMLElement;

    const product2QuantityContainer =
      within(product2CheckoutLine).getByText(/Quantity:/i).parentElement!;
    expect(product2QuantityContainer).toHaveTextContent("1");

    const product2SubtotalContainer =
      within(product2CheckoutLine).getByText(/Subtotal:/i).parentElement!;
    expect(product2SubtotalContainer).toHaveTextContent(
      String(mockProducts[1].price.gbp)
    );
  });
});
